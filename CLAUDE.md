...
- **Amazon-style chrome**: `navbar` is a navy (`bg-navy`) top bar — logo, big centered `search-bar` (white, gray category dropdown, amber Search button), language toggle, account, wishlist, cart-with-count — plus a secondary `#22407a` strip with an "All" menu + quick category links. `footer` is the Amazon-style dark-navy footer (Back-to-top bar, link columns, payments). `components/language-provider.tsx` is a lightweight EN/বাংলা i18n (`useLang().t()`, localStorage-persisted) used across the chrome. Homepage = admin-managed `sections/hero-carousel` (image banners, parallax, auto-slide) + `trust-bar`, `shipping-showcase` (air/sea cards + 3D `route-visualization`), `why-chinacart` (comparison table), category/deal/best-seller/trending rows, how-it-works, testimonials, suppliers, newsletter — all white "card" rows on `#f3f3f3`. Product + category cards use `tilt-card` `hoverOnly` (pointer 3D tilt, no scroll-reveal). `components/recently-viewed.tsx` (+ `lib/recently-viewed.ts`, localStorage) tracks/renders recently-viewed products on the home + product pages.
- **Hero banners (admin-managed)**: `Banner` DB model; `lib/banners.ts` (types + `DEFAULT_BANNERS` fallback) + `lib/banners-server.ts` (`getActiveBanners`/`getAllBanners`, DB-fail → defaults). `/admin/banners` (`banners-manager.tsx`) adds/edits/reorders/activates/deletes banners (image URL + title/subtitle/CTA); `/api/admin/banners` GET/POST/PATCH/DELETE (admin). The carousel renders banner images via plain `<img>` (any host, lazy) so no `next.config` allow-listing is needed; direct file upload needs a blob-storage provider (URL paste for now).
