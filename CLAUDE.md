...
- **Premium storefront**: `navbar` is a navy (`bg-navy`) bar — logo, big centered `search-bar` (white field, gray category dropdown, search), language toggle, account, wishlist, cart-with-count — plus a `#22407a` secondary strip ("All" + quick links). `components/language-provider.tsx` = lightweight EN/বাংলা i18n (`useLang().t()`, localStorage). Homepage = admin-managed `sections/hero-carousel` (image banners, parallax, auto-slide, **magnetic** CTA) + `trust-bar`, `shipping-showcase` (air/sea cards + 3D `route-visualization`), `why-chinacart` comparison, category/deal/best-seller/trending rows, how-it-works, testimonials, suppliers, newsletter — white "card" rows on `#f3f3f3`. Product + category cards use `tilt-card` `hoverOnly` (pointer 3D tilt, no scroll-reveal). `components/magnetic.tsx` = magnetic hover wrapper. `components/recently-viewed.tsx` (+ `lib/recently-viewed.ts`) tracks/renders recently-viewed products.
