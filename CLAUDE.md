...
- `app/` — App Router pages: `/`, `/products`, `/products/[id]`, `/categories`, `/how-it-works`, `/about`, `/contact`, `/auth`, `/suppliers`. Root layout wires the **language provider**, cart provider, premium navy navbar, footer, cart drawer and mobile bottom nav.
- **Theme**: burnt-orange + navy premium palette (chinacart.com.bd inspired). Tokens in `app/globals.css` `:root` — `--primary` #C75B39 (burnt orange), `--navy` #1a2032, `--accent` #fff3ee (light orange tint), `--secondary`/`--muted` #f5f5f5, `--foreground` dark navy, `--destructive` #e74c3c. Tailwind `brand` = burnt-orange family (#C75B39, hover `brand-600` #A84830). Stars use amber; product price + CTAs use `primary`.
- **Premium storefront**: `navbar` is a clean **white** bar (`bg-background`, border-b) — logo (navy + burnt-orange wordmark), big centered `search-bar` (white field + light border + burnt-orange Search button), language toggle, account, wishlist, cart-with-count (orange badge) — plus a light `bg-secondary` secondary strip ("All" + quick links, orange hover). `components/language-provider.tsx` = lightweight EN/বাংলা i18n (`useLang().t()`, localStorage). Homepage order: admin-managed `sections/hero-carousel` (image banners, parallax, auto-slide, **magnetic** CTA) → `trust-bar` → trending / best-seller / new-arrival product grids → how-it-works → `why-chinacart` comparison → testimonials → suppliers → shop-by-category → newsletter — white "card" rows on `#f3f3f3`. (Flash deals + the 3D shipping route were removed from the homepage; `flash-deals`/`shipping-showcase`/`route-visualization` components remain in the repo, unused.) **Scroll animations** via `components/scroll-reveal.tsx` — `ScrollReveal` (fade+zoom 0.96→1+rise), `RevealHeading` (slide + 3D depth), `StaggerGrid` (product cards zoom 0.9→1 + rise + rotateX, staggered); all `viewport.once` and reduced-motion-safe. Product + category cards use lightweight **CSS hover** (lift + shadow) — the JS 3D tilt (`tilt-card`) and per-image scroll parallax (`parallax-image`) were removed from cards for click/tap responsiveness (those files remain, unused). `components/magnetic.tsx` = magnetic hover wrapper (hero CTA). Global `touch-action: manipulation` + `cursor` rules in `globals.css` keep taps instant (no 300ms delay); the `Button` base is `cursor-pointer`. `components/recently-viewed.tsx` (+ `lib/recently-viewed.ts`) tracks/renders recently-viewed products.
- **Hero banners (admin)**: `Banner` model; `lib/banners.ts` (+ `DEFAULT_BANNERS` fallback) + `lib/banners-server.ts` (`getActiveBanners`/`getAllBanners`). `/admin/banners` (`banners-manager.tsx`) add/edit/**drag-reorder**/activate/delete (image URL + title/subtitle/CTA); `/api/admin/banners` GET/POST/PATCH/DELETE. Carousel uses plain `<img>` (any host, lazy) — no `next.config` allow-listing; file upload needs a blob provider (URL paste for now).
- **Logo & branding (admin)**: `BrandingSettings` singleton (logo data URLs + brand colours); `lib/branding.ts` (defaults, `LOGO_SLOTS`, `logoFor`, `hexToHslTriplet`, `brandColorCss`) + `lib/branding-server.ts` (`getBranding`). `/admin/branding` (`branding-manager.tsx`) = **drag-and-drop** upload for 5 logo slots (main/icon/footer/loading/invoice, stored as data URLs ≤3MB), live previews (navbar/hero/footer/loading/invoice/favicon, light+dark), and navy/accent **colour pickers**. `/api/admin/branding` GET/POST (admin). Root layout loads branding, injects a `<style>` overriding `--primary`/`--navy` (tailwind `navy` is now `hsl(var(--navy))`), and wraps the app in `BrandingProvider`; `BrandLogo` (in navbar/footer/invoice/receipt) shows the uploaded logo else the original CSS/text logo; favicon syncs from the icon logo.
...
- **Checkout advance rules** (`app/checkout`): no full COD — a minimum advance (default 30%, admin-set) is required; options 30/50/70/100% + custom, remainder collected as COD. Customer picks a method (bKash/Nagad/Rocket/Bank), pays to the merchant number / bank account and enters the TrxID; `/api/orders` validates the advance (≥ min, ≤ total) + TrxID, creates the order, records an advance `Payment`, sets `paymentPlan`, and (bank → `pending`/await-verify, else `confirmed`). Order confirmation + customer order page show paid-now / remaining-COD.
- **Payment settings** (`/admin/payment-settings`): `PaymentSettings` singleton (min advance %, method enable toggles, bKash/Nagad/Rocket numbers, bank details). `lib/payment-settings.ts` (+ `CHECKOUT_METHODS`, `enabledMethods`) + `lib/payment-settings-server.ts`; `/api/payment-settings` GET (public, used by checkout) + POST (admin). Admin order PaymentPanel has **Mark COD Collected** (records the remaining as a cash payment).
- **Gateway** (`lib/payment-gateway.ts`): abstraction + clear swap-in for real bKash/Nagad PGW — automated mode activates with merchant env credentials + egress; default is manual TrxID confirmation. Online auto-PGW + customer account portal + automated SMS/WhatsApp remain deferred (deep-link reminders only).
